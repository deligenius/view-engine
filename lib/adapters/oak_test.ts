...
Deno.test({
  ...
  async fn() {
    ...
    setTimeout(async () => {
      const actual = await fetch('http://localhost:8000/denjuck').then(res => res.text())
...
Deno.test({
  ...
  async fn() {
    ...
    setTimeout(async () => {
      const actual = await fetch('http://localhost:8000/ejs').then(res => res.text())
...
Deno.test({
  ...
  async fn() {
    ...
    const { signal } = controller
    const app = new Application();
    const ejsEngine = await engineFactory.getHandlebarsEngine();
    app.use(viewEngine(oakAdapter, ejsEngine));
    ...
    setTimeout(async () => {
      const actual = await fetch('http://localhost:8000/handlebars').then(res => res.text())
